...
def request(path, body=None, method=None):
    ...
    try:
        ...
        raise RuntimeError(f"{req.method} {path}: {error.code} {error.read().decode()}") from error


def download(run_id, artifact_id):
...
if mode == "prepare":
    ...
    for report_type, report_mode, selected, count in [
        ...
        if report_mode == "bulk":
            ...
            content = download(run_id, bundles[0]["id"])
            ...
            content = download(run_id, pdfs[0]["id"])
